...
HYPHY_CMD="hyphy hyphy/tests/hbltests/libv3/FitMG94upd.bf"
...
for CODON_FILE in "$PAL2NAL_OUTPUT_DIR"/*.fasta; do
    ...
    OUTPUT_FILE="$ABSREL_OUTPUT_DIR/${BASENAME}_fitmg94.json"
    LOG_FILE="$ABSREL_OUTPUT_DIR/${BASENAME}_fitmg94.log"
    ...
    if [ -f "$TREE_FILE" ]; then
        ...
        if [ $? -eq 0 ]; then
            echo "MG94 analysis completed for $CODON_FILE. Output saved to $OUTPUT_FILE and log saved to $LOG_FILE"
        else
            echo "Error running MG94 for $CODON_FILE. Check log file: $LOG_FILE"
        fi
    else
...
echo "All MG94 analyses completed. Results are in $ABSREL_OUTPUT_DIR."
