...
for CODON_FILE in "$PAL2NAL_OUTPUT_DIR"/*.fasta; do
    ...
    OUTPUT_FILE="$ABSREL_OUTPUT_DIR/${BASENAME}_absrel.json"
    LOG_FILE="$ABSREL_OUTPUT_DIR/${BASENAME}_absrel.log"
    ...
    if [ -f "$TREE_FILE" ]; then
        ...
        echo "Running aBSREL for $CODON_FILE with tree $TREE_FILE..."
        $HYPHY_CMD --alignment "$CODON_FILE" "ENV=TOLERATE_NUMERICAL_ERRORS=1;" --tree "$TREE_FILE"  --output "$OUTPUT_FILE" > "$LOG_FILE" 2>&1

        if [ $? -eq 0 ]; then
            echo "aBSREL analysis completed for $CODON_FILE. Output saved to $OUTPUT_FILE and log saved to $LOG_FILE"
        else
            echo "Error running aBSREL for $CODON_FILE. Check log file: $LOG_FILE"
        fi
    else
...
echo "All aBSREL analyses completed. Results are in $ABSREL_OUTPUT_DIR."
