...
PAL2NAL_OUTPUT_DIR="batstring_pal2nal" # Directory with PAL2NAL alignments of genes undergoing positive selection
...
for CODON_FILE in "$PAL2NAL_OUTPUT_DIR"/*.fasta; do
    ...
    if [ -f "$TREE_FILE" ]; then
        ...
        $HYPHY_CMD --alignment "$CODON_FILE" --tree "$TREE_FILE" --branches Background --error-sink Yes --output "$OUTPUT_FILE" > "$LOG_FILE" 2>&1
